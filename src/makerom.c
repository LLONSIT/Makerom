...
#include <fcntl.h>
#include "makerom.h"
#include "makerom_common.h" //Some nice vars and macros

const char *sys_errlist[];

//makes sense
static unsigned char* gloadFindFile(unsigned char* fullpath, unsigned char* postRootSuffix, unsigned char* fname);
...
*
*
*              Verifying the version of ido.
*  
* The IDO package apparently manages to leave a kind of record for the showprods command.
*
*  
*
...
void printVersion(void) {
    if (irixVersion == IRIX_VERSION_53) { //Minimum version
...
/********************************************************
*
*    Functions series "getThing"
*
*    This type of functions Do the same check if X file exists
*
*
*
******************************************************/
...
void getBootFile(u8* bootFileName) {
    s32 bootFd;
    s8 scratchFileName[255];
    struct stat buf;
    s8 errMessage[255];

    if ((bootFileName == NULL) && (gloadFindFile(&scratchFileName, "/usr/lib/PR", "Boot") != 0)) {
...
//Consider this as some kind of exception
...
*
*
*   Main function used for this series.
*
*   with two environment variables, like ROOT or also WORKAREA
*
*
*
*************************************************/
...
//This function has some nice UB's
